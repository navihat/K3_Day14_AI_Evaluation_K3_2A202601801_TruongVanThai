...
class RAGASEvaluator:
    ...
    @staticmethod
    def _overlap_ratio(covered: set[str], reference: set[str]) -> float:
        ...
        if not reference:
            return 1.0
        ratio = len(covered & reference) / len(reference)
    ...
    def evaluate_faithfulness(self, answer: str, context: str) -> float:
        ...
        """
        return self._overlap_ratio(_tokenize(context), _tokenize(answer))
    ...
    def evaluate_relevance(self, answer: str, question: str) -> float:
        ...
        Returns:
            float in [0.0, 1.0]
        """
        return self._overlap_ratio(_tokenize(answer), _tokenize(question))
    ...
    def evaluate_completeness(self, answer: str, expected: str) -> float:
        ...
        Returns:
            float in [0.0, 1.0]
        """
        return self._overlap_ratio(_tokenize(answer), _tokenize(expected))
    ...
    def evaluate_context_recall(self, contexts: list[str], expected: str) -> float:
        ...
        return self._overlap_ratio(union_tokens, _tokenize(expected))
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "You are an impartial evaluator. Score the ANSWER against each rubric "
            "criterion on a 0.0-1.0 scale.\n\n"
            f"QUESTION:\n{question}\n\n"
            f"ANSWER:\n{answer}\n\n"
            f"RUBRIC CRITERIA:\n{criteria_block}\n\n"
            "Respond with a JSON object mapping every criterion name to its score, "
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        overall = sum(per_entry) / len(per_entry)
        rest = per_entry[1:]
        positional = bool(rest) and per_entry[0] > (sum(rest) / len(rest)) + 0.1
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_context_recall": optional_mean(
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        if sum(1 for v in scores.values() if v <= lowest + 0.05) > 1:
    ...
    def generate_improvement_suggestions(
        ...
        playbook = {
            "hallucination": (
                "Add a grounding check that rejects claims whose tokens are absent "
                "from the retrieved contexts before returning the answer"
            ),
            "irrelevant": (
                "Rewrite the system prompt to restate the user question and require "
                "the answer to address it directly"
            ),
            "incomplete": (
                "Increase retriever top_k and chunk size so multi-condition policies "
                "are not split across missing chunks"
            ),
            "off_topic": (
                "Add intent routing that classifies the question against the corpus "
                "topics before generation"
            ),
            "refusal": (
                "Loosen the refusal guardrail so in-scope questions with partial "
                "evidence are answered with caveats instead of declined"
            ),
            "unknown": (
                "Instrument the pipeline to record a failure_type for every failing "
                "case so root causes can be clustered"
            ),
        }

        counts = self.categorize_failures(failures)
        suggestions = [
            playbook.get(failure_type, playbook["unknown"])
            for failure_type, _ in sorted(counts.items(), key=lambda kv: -kv[1])
        ]

        fallbacks = [
            "Add few-shot examples of well-grounded answers to the generation prompt",
            "Expand the golden dataset with the failing cases so regressions are caught early",
            "Calibrate the pass threshold against human labels before using it as a CI gate",
        ]
